...
- 존재하지 않는 경로와 404
- `/information/kartz/user`: 제목만 있는 미구현 화면의 라우트와 챗봇 추천을 제거했다. 직접 접근은 공통 404 안내와 `noindex`로 처리한다.
- `/post/9999-99-99*` 및 존재하지 않는 게시글: 공개 카탈로그에서 제외하고 직접 접근은 공통 404 안내와 `noindex`로 처리한다. 임시 원문과 첨부파일은 배포 번들에서도 제외한다.

위 URL은 계속 사용할 수 있고 링크를 따라갈 수 있도록 `noindex, follow`로 두며 사이트맵에서도 제외한다.

단, 미구현 카르츠 개인 기록과 임시·존재하지 않는 글은 기능을 제공하는 화면이 아니라 404 안내로 처리한다. 클라이언트의 404 안내와 서버의 HTTP 404 응답은 별개이며, 배포 호스팅에서 상태 코드를 확인해야 한다. `noindex` 자체를 애드센스 심사 제외 수단으로 간주하지 않는다.

## 공개 경로 검증

- 게시글 목록·상세·이전/다음은 `publicPosts.js`의 동일한 공개 카탈로그를 사용한다.
- 이미지·첨부파일은 `postAssets.js`에서 공개 글의 지정 확장자만 포함한다. 동적 `new URL`이 임시 Markdown까지 자산으로 포함하던 처리를 제거했다.
- 폐기된 `/information/ssc`는 프리렌더 대상에서 제외하고 실제 공개 경로인 `/information/appearance`를 포함한다.
- 빌드 후 `node scripts/verify-public-content.mjs`로 세 언어의 공개 글·임시 글·없는 글·직접 접근·클라이언트 이동·robots·사이트맵·번들 제외를 검증한다.
