...
export default function AttendanceVoteCreator() {
    ...
    const saveToDatabase = useCallback(async () => {
        if (!vote.title.trim()) return window.alert("투표 제목을 설정하세요");
        if (vote.choices.some(c => c.content.length === 0)) return window.alert("모든 항목의 내용을 작성하세요");
    ...
    const copyUuidToClipboard = useCallback(()=>{
        copyToClipboard(uuid, "투표ID가 복사되었습니다\n원하는 곳에 붙여넣으세요");
    }, [uuid]);
    const copyLinkToClipboard = useCallback(()=>{
        copyToClipboard(`${window.location.origin}/vote/cast`, "공유 링크가 복사되었습니다\n원하는 곳에 붙여넣으세요");
    }, []);
        ...
        <div className="row mt-4">
            ...
            <div className="col-sm-9 d-flex align-items-center">
                ...
                {vote.uuid.length === 0 ? (
                    ...
                    <button className="btn btn-primary ms-4 me-2 text-nowrap" onClick={copyUuidToClipboard}>
                        <FaCopy className="me-2" />
                        <span>ID복사</span>
                    </button>
                    <button className="btn btn-primary ms-4 me-2 text-nowrap" onClick={copyLinkToClipboard}>
                        <FaShare className="me-2" />
                        <span>공유주소복사</span>
                    </button>
                    <button className="btn btn-danger text-nowrap" onClick={createVote}>
